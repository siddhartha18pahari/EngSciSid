...
def plot_fit(my_func, xdata, ydata, xerror=None, yerror=None, init_guess=None, font_size=14,
    ...
    ax1.errorbar(xdata, ydata, yerr=yerror, xerr=xerror, fmt="o", markersize=5, capsize=3, label="Data", color="blue",
                 lw=1)
    ...
    ax1.set_title("Period(s) VS. Release Angle(Radian) of a Pendulum", fontsize=font_size + 2)
...
# Given data points with asymmetry introduced
xdata = [1.89, 1.26, 1.05, 1.02, 1.03, 1.10, 1.15, 1.63]  # These are your y-values in the original data
ydata = [np.deg2rad(80), np.deg2rad(60), np.deg2rad(40), np.deg2rad(20), np.deg2rad(-20), np.deg2rad(-40),
         np.deg2rad(-60), np.deg2rad(-80)]  # Converted x-values to radians

yerror = [0.05] * len(ydata)  # Swapped x and y errors
xerror = [0.63, 0.41, 0.34, 0.25, 0.26, 0.36, 0.44, 0.64]  # Swapped x and y errors

plot_fit(function, xdata, ydata, xerror, yerror, xlabel="Period (s)", ylabel="Angle (radians)")
